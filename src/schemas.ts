...
const userId = z.string("User ID is required").cuid2({ error: "Invalid user ID format" }).openapi({
...
const userName = z
  ...
  .nonempty({ error: "Name must not be an empty string" })
  .openapi({ description: "The user's name for display" });

const email = z.string("Email is required").email({ error: "Invalid email address" }).openapi({
...
const sessionToken = z
  .string("Session token is required")
  .nonempty({ error: "Session token can't be empty" })
  .openapi({ description: "The admin user's session token" });

const createdAt = z
  .string("Created datetime is required")
  .datetime({ error: "Invalid ISO 8601 created datetime" })
  .openapi({
    description: "The date and time the record was created in ISO 8601 format",
  });

const updatedAt = z
  .string("Updated datetime is required")
  .datetime({ error: "Invalid ISO 8601 updated datetime" })
  .openapi({
    description: "The date and time the record was last updated in ISO 8601 format",
  });

const apiKey = z
  .string("API key in Authorization header is required")
...
const model = z.string("Model identifier is required").openapi({
...
const modelProvider = z
  .string("Model provider is required")
...
export const UserIdKeyIdReqParamsSchema = z
  .object({
    userId,
    keyId: z.string("Key ID is required").cuid2({ error: "Invalid key ID format" }).openapi({
...
export const CreateUserReqSchema = z
  .object({
    ...
    email,
    keyExpiresAt: z
      .string("Key expires at is required")
      .datetime({ error: "Key expires at must be a valid ISO 8601 datetime", offset: true })
      .openapi({
...
export const CompletionReqSchema = z
  .object({
    model,
    prompt: z.string("Prompt is required").openapi({
  ...
  .object({
    ...
    messages: z
      .array(
        z.object({
          role: z
            .enum(["user", "assistant"], { error: "Role is required" })
            .openapi({ description: "The role of the message" }),
          message: z.string("Message is required").openapi({
